...
msg_tabicl_not_available <- function(cnd) {
  c(
    x = "The {.pkg tabicl} Python package is not installed in the discovered Python installation ({.file {reticulate::py_exe()}}).",
    i = 'Allow reticulate to automatically configure an ephemeral Python environment by
         removing the Python installation from the order of discovery and restarting the R session.
         See {.href [Order of Discovery](https://rstudio.github.io/reticulate/dev/articles/versions.html#order-of-discovery)} for more info.',
    i = 'Or install {.pkg tabicl} into the selected Python environment by calling
        {.code reticulate::py_install("tabicl")}'
  )
}
  ...
  if (any(outside_py_env)) {
    cli::cli_abort(
      c(
        ...
        i = "See {.url https://github.com/ielbadisy/tfmr/issues}.",
